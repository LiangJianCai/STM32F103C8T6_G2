...
ErrorCode_t MyRTC_SetTime(void)
{
	...
	if (MyRTC_Time[1] < 1 || MyRTC_Time[1] > 12) {
		REPORT_ERROR(ERROR_OUT_OF_RANGE, ERROR_LEVEL_ERROR, MODULE_ID_RTC, "Month out of range");
		return ERROR_OUT_OF_RANGE;
	}
	...
	if (time_cnt == -1) {
		REPORT_ERROR(ERROR_INVALID_PARAM, ERROR_LEVEL_ERROR, MODULE_ID_RTC, "Invalid time format");
